...
Optional `~/.config/wrk/settings.toml`. Key fields: `claude_command` (defaults to `["claude"]`) and `shell_command` (falls back to `$SHELL`, then `/bin/bash`). Used to support quirky setups like `["steam-run", "claude"]` on NixOS. `claude_base()` strips a trailing `--continue` for backwards compat with older configs. `confirm_quit` (default true) gates app exit behind a confirmation modal — both quit paths (`GlobalAction::Quit` and `q` in the projects pane) route through `App::request_quit`, which opens `ModalState::ConfirmQuit` or sets `should_quit` directly.

**Harnesses.** A tab runs a coding-agent *harness* — `HarnessKind` (`Claude` | `Kimi`). Claude is the default and always enabled; others opt in via `[harness.<kind>]` (`enabled`, custom `command`) plus `default_harness` for new tabs. `Settings::harness_command(kind)` resolves the launch command (Claude: `[harness.claude].command` → legacy `claude_command` → `["claude"]`, trailing `--continue` stripped; Kimi: `[harness.kimi].command` → `["kimi"]`); `harness_enabled` (claude on, kimi off by default), `enabled_harnesses`, `default_harness()` (falls back to an enabled harness), and `harness_pane_env` (Kimi → `KIMI_DISABLE_TELEMETRY=1`, so a machine with no Kimi account doesn't stall on the update/telemetry check). The harness flows settings → `store::SessionRef.harness` (persisted; omitted from TOML for Claude) → `ClaudeTab.harness` (the type keeps its historical name but hosts any harness) → the spawn command builders.

Command builders are `new_agent_command`/`resume_agent_command`/`continue_agent_command`, dispatched per tab by `spawn_command_for_tab`. **Claude** resumption is id-pinned and never uses `--continue`: it mints a UUID at first spawn (`claude --session-id <uuid> --name <project>`), persists it, and resumes via `claude --resume <id>` — no post-spawn filesystem detection. **Kimi** can't pre-mint an id. The intended path is to *learn* the id from Kimi's `SessionStart`/status hooks (`--harness kimi` → `handle_status_update` records + persists it → resume via `kimi --session <id>`); **but Kimi 2.0.0 does not execute `config.toml` hooks at all** (verified: a full turn ran and no hook fired), so in practice a Kimi tab has no learned id and resumes via **`kimi --continue`** (the directory's latest session, or a fresh one if none). The transient `ClaudeTab.fresh` flag (true only for a picker-added tab, cleared after the first spawn) is what distinguishes "new Kimi session" (bare `kimi`) from "reopen" (`--continue`) when there is no learned id. The hook path stays wired and takes over automatically if a future Kimi build fixes hook execution. Limitation: multiple Kimi tabs in one project all `--continue` the same latest session. When hooks don't fire, the sidebar status also falls back to the idle-time heuristic. New tabs pick a harness in the `ClaudeTabPickerModal` (←/→ cycles the enabled harnesses; hidden when only one is enabled — Kimi has no on-disk session discovery yet, so it offers only "New session").
...
1. **Precise**: agent hooks pushed over the socket — no files, no polling. When wrk spawns an agent PTY it exports `WRK_TAB=<status_id>` (an opaque per-tab id, `ClaudeTab.status_id`) alongside `WRK_SOCK` (see IPC below). The hook commands run `wrk hook <kind>` (`cmd_hook`), which reads `WRK_SOCK`/`WRK_TAB` and sends an `ipc::StatusUpdate { tab, kind, session_id }`; the event loop drains it (`App::handle_status_update`) and folds it into the matching tab's `status: TabStatus`. Kinds (`StatusKind`): `busy`, `stopped`, `waiting`, `subagent-start`, `subagent-stop` (the last two move a per-tab sub-agent counter). `TabStatus.event` maps Busy→busy dot, Stopped→green, Waiting→red; it stays `None` until the first hook, so a fresh tab uses the fallback. Commands are guarded with `[ -n "$WRK_SOCK" ] && "$WRK_BIN" hook <kind> …; true` — the `; true` keeps exit 0 for sessions not launched by wrk; `wrk hook` itself is best-effort and never fails. **`$WRK_BIN` is exported by the running instance** (its own `current_exe()`, via `base_pty_env`) — hooks and skills never hardcode a path, so the config stays portable across machines and correct under `cargo run`. `pid_is_alive` (a `/proc` check) backs the startup artifact sweeps.

   **Per-harness hook install.** `install-hooks` writes hooks + skills for every *enabled* harness; `uninstall-hooks` removes both harnesses' entries regardless (cleanup).
   - **Claude** (`HOOKS`, `install_hooks`/`install_skills`): JSON in `~/.claude/settings.json`; events UserPromptSubmit→busy, Stop→stopped, Notification→waiting, PreToolUse/`Task`→subagent-start, SubagentStop→subagent-stop. Skills in `~/.claude/skills/<name>/SKILL.md`, `/`-invoked.
   - **Kimi** (`KIMI_HOOKS`, `install_kimi_hooks`/`install_kimi_skills`; home `$KIMI_CODE_HOME` else `~/.kimi-code`): a TOML `[[hooks]]` array in `config.toml`, merged with **`toml_edit`** so the user's hand-written provider/model config and comments survive. Events SessionStart→stopped, UserPromptSubmit→busy, Stop→stopped, PermissionRequest→waiting, PermissionResult→busy, SubagentStart/Stop. Every Kimi hook command carries `--harness kimi`, which makes `cmd_hook` read the event JSON on stdin (`read_stdin_session_id`) and forward Kimi's `session_id`; `handle_status_update` adopts it onto the Kimi tab and persists it (Claude ignores a pushed id — wrk owns its). Skills in `~/.kimi-code/skills/<name>/SKILL.md` with Kimi frontmatter (`name`/`description`/`whenToUse`, no `allowed-tools`), `/skill:<name>`-invoked; `end-local-review` runs `"$WRK_BIN" review end` itself (Kimi has no `!`…`` injection).
...
`install_hooks` merges entries into the user's existing `settings.json` and re-running it refreshes the command (so fixes propagate) and **upgrades legacy file-polling installs in place**. Entries are detected by marker substring (`command_is_ours` matches the current `WRK_SOCK` commands and the legacy `WRK_STATUS_FILE` ones), not by exact-match — preserve a marker when editing; the same marker identifies our Kimi `[[hooks]]` entries for refresh/removal. `uninstall_hooks` removes any marked entry (current or legacy) across all hook events. There are no status files to create or clean up.

The `install-hooks` subcommands also write/remove wrk's **skills**, one dir per entry of `skill_specs()`/`kimi_skill_specs()` (shared writer `write_skills_in`): `wrk-view` (open a file), `start-local-review` and `end-local-review` (the code-review commands, below). Each SKILL.md carries a marker comment so uninstall never deletes a user's own same-named skill. A skill's directory name is its command name (`start-local-review` → `/start-local-review` for Claude, `/skill:start-local-review` for Kimi).

### `wrk view` and IPC (`src/ipc.rs`)

Each running TUI binds a per-instance Unix socket at `<runtime>/wrk/sock/wrk-<pid>.sock` (`ipc::serve`), stored on `App.socket_path` and exported to every spawned PTY (Claude **and** shell) as `WRK_SOCK`, alongside `WRK_PROJECT` and `WRK_BIN` (this instance's `current_exe()`, so hooks/skills invoke `"$WRK_BIN" …` — see `App::base_pty_env`); Claude tabs additionally get `WRK_TAB`. Stale sockets and review mirrors from instances that exited uncleanly are reaped once at startup (`ipc::sweep_stale_sockets` / `review::sweep_stale_mirrors`, keyed on a dead owning pid). The wire message is one line of JSON, an internally-tagged (`cmd`) `ipc::Request` enum: `Open(OpenRequest)` from `wrk view`, `Status(StatusUpdate)` from `wrk hook`, and `Review(ReviewRequest)` from `wrk review` (see Code review below). The event loop drains the shared `Receiver<Request>` and routes each via `App::handle_request` → `handle_open_request` / `handle_status_update` / `handle_review_request`. `wrk view <file>` (`cmd_view`) canonicalizes the path; if `WRK_SOCK` is set and connectable it sends `Request::Open` which becomes a markdown tab in the named project (`add_markdown_tab`); otherwise it execs the `wrk-md` pager (sibling of the current exe, else `PATH`). The socket file is removed on shutdown. `ipc::socket_dir()` is `status::runtime_dir()/sock`.

### Code review (`src/review/`, `src/ui/review.rs`)

A GitHub-style local diff review, driven from a Claude session. **Diff engine** (`review::diff`): git supplies the file list (`git diff --name-status`, plus untracked as `Added` for the working-tree target) and each side's bytes (`git show <rev>:<path>` / the working file); `similar` aligns lines into side-by-side `DiffRow`s; long unchanged runs become revealable `Collapsed` `Segment`s (keeping `CONTEXT` lines of lead/trail). All git runs from the repo root that `resolve_repo_root(dir)` derives (`git rev-parse --show-toplevel`) from the *invocation directory* — the CWD `wrk review start` ran in, carried on `ReviewRequest.cwd` — so a worktree nested inside a non-git project directory reviews correctly (the project dir may be a bare container of clones/worktrees); it falls back to the project path when the CWD isn't in a repo. `DiffTarget` is `WorkingVsHead | WorkingVs(rev) | Range(a,b)` and parses the optional `wrk review start [target]` arg. Non-UTF-8 either side → `binary` (no textual diff).
...
**Flow & reporting.** `wrk review` is a real clap subcommand (`ReviewAction::Start { target } | End`, not a raw arg vec) so `wrk review start --help` prints help and a `target` beginning with `-` is rejected before anything opens. `wrk review start [target]` (`cmd_review`) sends `Request::Review(Start)` (tab=`WRK_TAB`, project=`WRK_PROJECT`, `cwd`=`current_dir()`); `handle_review_request` resolves the repo root from `cwd` and builds the diff, opening the overlay. Comments are **pulled**, not pushed (there's no supported channel into a live Claude session): on every edit `App::persist_review_comments` mirrors them to `runtime_dir()/review/<key>.json` (`key` = `<pid>-<WRK_TAB else project>` — pid-prefixed so concurrent instances don't share a file and a new instance never reads a dead predecessor's mirror, since `WRK_TAB` like `tab0` repeats per process; `review_mirror_key`/`review_env_key`, `review::write/read/remove_mirror`). `wrk review end` reads that file, prints the comments as markdown (`format_review_comments`) for Claude to ingest, and sends `Request::Review(End)` (closes the overlay, removes the mirror). The `start-local-review` (model-invocable — the agent inspects git and picks the target) and `end-local-review` (`!`wrk review end`` injection) skills wrap the two subcommands.
